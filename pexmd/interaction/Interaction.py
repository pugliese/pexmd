...
coul = ct.CDLL('pexmd/interaction/coulomb.so')

coulforces_c = coul.forces
coulforces_c.argtypes = [ct.c_voidp, ct.c_voidp, ct.c_longlong,
                            ct.c_float, ct.c_float, ct.c_voidp]
coulforces_c.restype = ct.c_float

coulpairforce_c = coul.pair_force
coulpairforce_c.argtypes = [ct.c_float, ct.c_float]
coulpairforce_c.restype = ct.c_float

coulpairenergy_c = coul.pair_energ
coulpairenergy_c.argtypes = [ct.c_float, ct.c_float, ct.c_float]
coulpairenergy_c.restype = ct.c_float

class Coulomb(ShortRange):
  """
  Coulomb potential
  """
  def __init__(self, rcut, k, shift_style='None'):
    self.k = k
    super().__init__(rcut, shift_style)
  ...
  def forces(self, x, v, pairs=None):
    ...
    pairsp = pairs.ctypes.data_as(ct.c_voidp)
    forcesp = forces.ctypes.data_as(ct.c_voidp)
    energ = coulforces_c(xp, pairsp, len(pairs), self.k, self.rcut, forcesp)
    return forces, energ
  ...
  def pair_force(self, s1, s2):
    ...
    if d > self.rcut:
      return np.zeros_like(s1)
    cf = coulpairforce_c(d, self.k)*(s1-s2)
    if self.shift_style == 'None':
      return cf
    elif self.shift_style == 'Displace':
      return cf
  ...
  def pair_energ(self, s1, s2):
    ...
    if self.shift_style == 'None':
      ...
      vcut = coulpairenergy_c(self.rcut, 0, self.k)
    ce = coulpairenergy_c(d, vcut, self.k)
    return ce

qcnm = ct.CDLL('pexmd/interaction/qcnm.so')

qcnmforces_c = qcnm.forces
qcnmforces_c.argtypes = [ct.c_voidp, ct.c_voidp, ct.c_longlong, ct.c_float,
                        ct.c_float, ct.c_float, ct.c_float, ct.c_float,
                        ct.c_float, ct.c_float, ct.c_float, ct.c_voidp]
qcnmforces_c.restype = ct.c_float

qcnmpairforce_c = qcnm.pair_force
qcnmpairforce_c.argtypes = [ct.c_float, ct.c_float, ct.c_float, ct.c_float,
                             ct.c_float, ct.c_float, ct.c_float, ct.c_float]
qcnmpairforce_c.restype = ct.c_float

qcnmpairenergy_c = qcnm.pair_energ
qcnmpairenergy_c.argtypes = [ct.c_float, ct.c_float, ct.c_float, ct.c_float,
                    ct.c_float, ct.c_float, ct.c_float, ct.c_float, ct.c_float]
qcnmpairenergy_c.restype = ct.c_float

class QCNM(ShortRange):
  """
  Morse potential
  """
  def __init__(self, rcut, Vo, r1, p1, r2, p2, d, a, shift_style='None'):
    self.Vo = Vo
    self.r1 = r1
    self.p1 = p1
    self.r2 = r2
  ...
  def forces(self, x, v, pairs=None):
    ...
    pairsp = pairs.ctypes.data_as(ct.c_voidp)
    forcesp = forces.ctypes.data_as(ct.c_voidp)
    energ = qcnmforces_c(xp, pairsp, len(pairs), self.Vo, self.r1, self.p1,
                        self.r2, self.p2, self.d, self.a, self.rcut, forcesp)
    return forces, energ
  ...
  def pair_force(self, s1, s2):
    ...
    if d > self.rcut:
      return np.zeros_like(s1)
    qf = qcnmpairforce_c(d, self.Vo, self.r1, self.p1,
                        self.r2, self.p2, self.d, self.a)*(s1-s2)
    if self.shift_style == 'None':
      return qf
    elif self.shift_style == 'Displace':
      return qf
  ...
  def pair_energ(self, s1, s2):
    ...
    if self.shift_style == 'None':
      ...
      vcut = qcnmpairenergy_c(self.rcut, 0, self.Vo, self.r1, self.p1,
                          self.r2, self.p2, self.d, self.a,)
    qe = qcnmpairenergy_c(d, vcut, self.Vo, self.r1, self.p1,
                        self.r2, self.p2, self.d, self.a,)
    return qe
