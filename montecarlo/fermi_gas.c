...
int muestrear_impulsos(struct Particles *parts, struct Pauli *pauli, struct Externos *params, int Nsamp, int factor){
  ...
  // Muestreo
  char filename[255];
  sprintf(filename, "impulsos_mx100_%f.txt", params->T);
  printf("%s\n", filename);
...
// Inicializacion
  ...
  save_checkpoint("checkpoint_mx100_5.txt", &parts, &pauli, &params);
  for (int k = 0; k < 6; k++) {
    params.T = 3 - 0.5*k;
    muestrear_impulsos(&parts, &pauli, &params, 100, 1);
  }
  save_checkpoint("checkpoint_mx100_0,5.txt", &parts, &pauli, &params);
  for (int k = 0; k < 6; k++) {
    params.T = 0.3 - 0.05*k;
    muestrear_impulsos(&parts, &pauli, &params, 200, 3);
  }
  save_checkpoint("checkpoint_mx100_0,05.txt", &parts, &pauli, &params);
  for (int k = 0; k < 6; k++) {
    params.T = 0.03 - 0.005*k;
    muestrear_impulsos(&parts, &pauli, &params, 200, 3);
  }
  save_checkpoint("checkpoint_mx100_0,005.txt", &parts, &pauli, &params);
  for (int k = 0; k < 6; k++) {
    params.T = 0.003 - 0.0005*k;
    muestrear_impulsos(&parts, &pauli, &params, 400, 10);
  }
  save_checkpoint("checkpoint_mx100_0,0005.txt", &parts, &pauli, &params);
  for (int k = 0; k < 6; k++) {
    params.T = 0.0003 - 0.00005*k;
    muestrear_impulsos(&parts, &pauli, &params, 400, 10);
  }
  save_checkpoint("checkpoint_mx100_0,00005.txt", &parts, &pauli, &params);

/*
  load_checkpoint("checkpoint_mx10_0,005.txt", &parts, &pauli, &params);
  for (int k = 0; k < 6; k++) {
    params.T = 0.003 - 0.0005*k;
    muestrear_impulsos(&parts, &pauli, &params, 2000, 10);
  }
  save_checkpoint("checkpoint_mx10_0,0005.txt", &parts, &pauli, &params);
*/
  free(parts.q);
  free(parts.p);
  return 0;
