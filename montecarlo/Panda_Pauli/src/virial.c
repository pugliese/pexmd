...
inline float fgorce_mod_pauli_norma1(float rsq, float *p1, float *p2, struct Pauli *pauli, float *gorce_mod){
  float f_mod;
  float r2_red = rsq/(pauli->qo*pauli->qo);
  if (r2_red < pauli->scut2){
    float r_red = sqrt(r2_red);
    float p_red = sqrt(distancia_p(p1, p2))/pauli->po;
    float exp_q = exp(-r_red);
    float exp_p = exp(-p_red);
    f_mod = pauli->D*exp_p*exp_q/(pauli->qo*pauli->qo*r_red);
    *gorce_mod = (pauli->D*exp_q - pauli->shift)*exp_p/(pauli->po*pauli->po*p_red);
  }else{
    f_mod = 0;
    *gorce_mod = 0;
  }
  ...
  *gorce_mod = 0;
  ...
  if (t1/2 == t2/2){
    ...
    if (t1 == t2){
      ...
      //force_mod += fgorce_mod_pauli_norma1(rsq, p1, p2, pot_tot->pauli, gorce_mod);
    }
  }else{
...
inline float fgorce_mod_QCNM(int t1, int t2, float rsq, float *p1, float *p2, struct Interaction *pot_tot, float *gorce_mod){
  float force_mod;
  *gorce_mod = 0;
  float r = sqrt(rsq);
  force_mod = force_mod_QCNM(r, pot_tot->qcnm);
  if (t1 == t2){
    force_mod += fgorce_mod_pauli(rsq, p1, p2, pot_tot->pauli, gorce_mod);
    //force_mod += fgorce_mod_pauli_norma1(rsq, p1, p2, pot_tot->pauli, gorce_mod);
  }
    ...
    for (int k = 0; k < 14; k++){
      ...
      while (i != -1){
        ...
        while (j !=-1 && j != i){
          ...
          force_mod = fgorce_mod_QCNM(parts->type[i], parts->type[j], rsq, parts->p+3*i, parts->p+3*j, pot_tot, &gorce_mod);
          for (int l = 0; l < 3; l++){
            qF += force_mod*delta_q[l]*delta_q[l];
            qp -= gorce_mod*delta_p[l]*delta_p[l];
          }
