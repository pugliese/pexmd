...
float delta_energia_pot_sin_LUT(struct Particles *parts, struct Interaction *pot_tot, float *new_q, float *new_p, int i, int *new_ms, int *ms, float* delta_pauli){
  ...
  double new_pot_pauli = 0, old_pot_pauli = 0;
  float rsq;
  int idxs[3];
  ...
  for(int k = 0; k < 27; k++){
    ...
    while (j != -1){
      if (j != i){
        ...
        old_pot = old_pot + interaction_sin_LUT(parts->type[i], parts->type[j], rsq, parts->p+3*i, parts->p+3*j, pot_tot, &old_pot_pauli);
      }
      j = parts->siguiente[j];
    }
    ...
    while (j != -1){
      if (j != i){
        ...
        new_pot = new_pot + interaction_sin_LUT(parts->type[i], parts->type[j], rsq, new_p, parts->p+3*j, pot_tot, &new_pot_pauli);
      }
      j = parts->siguiente[j];
    }
  }
...
float delta_energia_pot_QCNM(struct Particles *parts, struct Interaction *pot_tot, float *new_q, float *new_p, int i, int *new_ms, int *ms, float* delta_pauli){
  ...
  for(int k = 0; k < 27; k++){
    ...
    while (j != -1){
      if (j != i){
        rsq = distancia(parts->q+3*i, parts->q+3*j, idxs, parts->l);
        old_pot = old_pot + interaction_con_QCNM(parts->type[i], parts->type[j], rsq, parts->p+3*i, parts->p+3*j, pot_tot, &old_pot_pauli);
      }
      j = parts->siguiente[j];
    }
    new_idx = (((new_ms[0]+idxs[0]+M) % M)*M + (new_ms[1]+idxs[1]+M) % M)*M + (new_ms[2]+idxs[2]+M) % M;
    ...
    while (j != -1){
      if (j != i){
        rsq = distancia(new_q, parts->q+3*j, idxs, parts->l);
        new_pot = new_pot + interaction_con_QCNM(parts->type[i], parts->type[j], rsq, new_p, parts->p+3*j, pot_tot, &new_pot_pauli);
      }
      j = parts->siguiente[j];
    }
  }
...
int step_QCNM(struct Particles *parts, struct Interaction *pot_tot, struct Externos *params){
  ...
  float delta_kin = delta_energia_kin(parts, new_p, i);
  float delta_nuc = delta_energia_pot_QCNM(parts, pot_tot, new_q, new_p, i, new_ms, ms, &delta_pauli);
  float delta_E = delta_kin + delta_nuc + delta_pauli;
  int acepto = (delta_E <= 0) || (uniform() < exp(-delta_E/params->T));
  if (acepto){
    new_m = (new_ms[0]*M + new_ms[1])*M + new_ms[2];
...
int N_steps_QCNM(struct Particles *parts, struct Interaction *pot_tot, struct Externos *params, int Nsteps){
  int aceptados = 0;
  for (int k = 0; k < Nsteps; k++) {
    aceptados = aceptados + step_QCNM(parts, pot_tot, params);
  }
  return aceptados;
}
