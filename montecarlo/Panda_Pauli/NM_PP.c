...
int armar_lista(struct Celdas *tipo, float rcut, float L){
  int idx, N = tipo->nc;
  tipo->M = (int) floor(L/rcut);
  tipo->l = L/tipo->M;
  int M3 = tipo->M*tipo->M*tipo->M;
  tipo->primero = (int *) malloc(M3*sizeof(int));
  tipo->siguiente = (int *) malloc(N*sizeof(int));
  tipo->anterior = (int *) malloc(N*sizeof(int));
  tipo->celda = (int *) malloc(N*sizeof(int));
  for(int i = 0; i < M3; i++){
    tipo->primero[i] = -1;
  }
  for(int i = 0; i < N; i++){
    ...
    for (int k = 0; k < 3; k++){
      idx = idx*tipo->M;
      idx += (int) floor(tipo->q[3*i+k]/tipo->l);
      tipo->q[3*i+k] -= tipo->l*floor(tipo->q[3*i+k]/tipo->l);
    }
    tipo->celda[i] = idx;
    tipo->siguiente[i] = tipo->primero[idx];    // Su siguiente es el primero previo (o -1 si no habia)
    tipo->anterior[i] = -1;              // Es el nuevo primero; no tiene anterior
    if (tipo->primero[idx] != -1){
      tipo->anterior[tipo->primero[idx]] = i; // Si habia primero previo, ahora tiene como anterior a i
    }
    tipo->primero[idx] = i;         // i es el nuevo primero
  }
  return 0;
}

int actualizar_lista(struct Particles *tipo, int i, int idx){
  int N = tipo->n;
  if (idx!=tipo->celda[i]){
    // Remuevo i de su celda anterior
    int sig_i = tipo->siguiente[i];
    int ant_i = tipo->anterior[i];
    if (sig_i != -1){
      tipo->anterior[sig_i] = ant_i;
    }
    if (ant_i != -1){
      tipo->siguiente[ant_i] = sig_i;
    }else{
      tipo->primero[tipo->celda[i]] = sig_i;
    }
...
int print_lista(struct Particles *tipo){
  int j, M3 = tipo->M*tipo->M*tipo->M, flag;
  for(int i = 0; i < M3; i++){
    j = tipo->primero[i];
    printf("Celda %d:\n%d", i, j);
    while(tipo->siguiente[j] != -1){
      printf(" -> %d", tipo->siguiente[j]);
      j = tipo->siguiente[j];
    }
    printf("\n%d", j);
    while(tipo->anterior[j] != -1){
      printf(" <- %d", tipo->anterior[j]);
      j = tipo->anterior[j];
    }
    printf("\n");
  }
...
int muestrear_todo(char *filename_termo, char *filename_dist, struct Particles *parts, struct Pauli *pauli, struct Externos *params, int Nsamp, int factor, int factor_term){
  ...
  for(int k = 0; k < Nsamp; k++){
    ...
    for(int l = 0; l < parts->n; l++){
      fprintf(f, "%d 1 %f %f %f %f %f %f\n", l, parts->q[3*l], parts->q[3*l+1], parts->q[3*l+2], parts->p[3*l], parts->p[3*l+1], parts->p[3*l+2]);
    }
  }
  fprintf(f, "\n");
  fclose(f);
...
int muestrear_estado(char *filename_termo, char *filename_conf, struct Particles *parts, struct Pauli *pauli, struct Externos *params, int Nsamp, int factor, int factor_term){
  // Termalizacion
  int segs = time(NULL);
  ...
  float ekin = 0, etot = 0, PyT_tot[2], PyT[2];
  PyT_tot[0] = 0, PyT_tot[1] = 0;
  FILE *f = fopen(filename_conf, "w");
  int aceptados = 0, m, mx, my, mz, M = parts->M;
  for(int k = 0; k < Nsamp; k++){
    ...
    fprintf(f, "ITEM: id type ATOMS x y z vx vy vz \n");
    for(int l = 0; l < parts->n; l++){
      m = parts->celda[l];
      mx = m/(M*M);
      my = (m/M) % M;
      mz = m % M;
      fprintf(f, "%d 1 %f %f %f %f %f %f\n", l, mx*parts->l+parts->q[3*l], my*parts->l+parts->q[3*l+1], mz*parts->l+parts->q[3*l+2], parts->p[3*l], parts->p[3*l+1], parts->p[3*l+2]);
    }
  }
  fclose(f);
  ekin = ekin/Nsamp;
...
// Particulas
  ...
  float delta_qo, delta_po;
  struct Pauli pauli;
// PARAMETROS 1; alta intensidad, bajo alcance en p
  if (P==1){
    pauli.qo = 2.15; // fm
    pauli.po = 37.33; // MeV/c
    pauli.D = 297;  // MeV
    pauli.scut2 = 10;
...
// PARAMETROS 2; intensidad intermedia, alcance en p intermedio
  if (P==2){
    pauli.qo = 2.15; // fm
    pauli.po = 50; // MeV/c
    pauli.D = 133.26;  // MeV
...
// PARAMETROS 3; menor intensidad, mayor alcance en p
  if (P==3){
    pauli.qo = 2.15; // fm
    pauli.po = 91.78; // MeV/c
    pauli.D = 70;  // MeV
    pauli.scut2 = 10;
    pauli.shift = pauli.D*exp(-0.5*pauli.scut2);
...
// PARAMETROS 4; mayor area excluida
  if (P==4){
    pauli.qo = 2.15; // fm
    pauli.po = 91.78; // MeV/c
    pauli.D = 70;  // MeV
...
// PARAMETROS 6; Maruyama modificado
  if (P==6){
    pauli.qo = 1.8; // fm
    pauli.po = 110; // MeV/c
    pauli.D = 300;  // MeV
...
// PARAMETROS 7; Dorso
  ...
  params.delta_q = delta_qo*pow(params.T/25.0, 0.8);
  params.delta_p = delta_po*pow(params.T/25.0, 0.8);
  ...
  float dT = 0.5;
  int segs, aceptados, Nsamp = 20, factor = 10, factor_term = 200;
  char filename_termo[255], filename_config[255];
  sprintf(filename_termo, "params_%d/termo_%.3f.txt", P, rho);
  FILE *f = fopen(filename_termo, "w");
  fprintf(f, "# T_MC\t\tEcin\t\tEpot\t\tEtot\t\tP\t\tT_V\t\tTiempo\n");
  fclose(f);
  N_steps(&parts, &pauli, &params, factor*parts.n);
  for (int k = 0; k < 56; k++){
    params.delta_q = delta_qo*pow(params.T/25, 0.8);
    params.delta_p = delta_po*pow(params.T/25, 0.8);
    segs = time(NULL);
    sprintf(filename_config, "params_%d/config_%.3f_%.3f.lammpstrj", P, rho, params.T);
    aceptados = muestrear_estado(filename_termo, filename_config, &parts, &pauli, &params, Nsamp, factor, factor_term);
    segs = time(NULL) - segs;
    printf("(rho = %1.2f|T = %1.2f) con %1.2f%% aceptados en %d segundos\n", rho, params.T, 100*((float) aceptados)/((factor_term*0+factor*Nsamp)*parts.n), segs);
    if (k == 30) dT = 0.25;
    if (k == 46) dT = 0.1;
    params.T -= dT;
  }
