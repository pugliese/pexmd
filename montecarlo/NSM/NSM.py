...
Ts = [float(f.split("_")[1][:-4]) for f in files]
n_Ts = len(Ts)
Ts = np.sort(Ts)
...
if(tipo == 't'):
  ...
  for k in range(n_Ts):
    data = np.loadtxt(caso+"energias_%f.txt" %(Ts[k]), dtype=np.float32)
    E_kin.append(data[:,0])
    E_nuc.append(data[:,1])
...
if(tipo == 'e'):
  ...
  for k in seleccion:
    data_aux = np.loadtxt(caso+"distribucion_%f.txt" %(Ts[k]), dtype=np.float32)
    data_q = data_aux[:, 0]
    data_p = data_aux[:, 1]
    plt.figure()
    for i in range(g):
...
if (tipo == "gr"):
  ...
  for k in seleccion:
    ...
    plt.xlabel(r"$r$")
    plt.ylabel(r"$g(r)$")
    plt.title(r"$T=0.5 MeV$ $T=%f MeV$" %(Ts[k]))
...
if (tipo == "ap"):
  ...
  P = np.zeros_like(Ts)
  for k in range(n_Ts):
    data = np.loadtxt(caso+'distribucion_%f.txt' %(Ts[k]), dtype=np.float32)
    ...
    P[k] = qF[k] + qp[k]
  np.savetxt(caso+"presiones_N=1000.txt", [Ts, P])


if (tipo=="vmd"):
  for k in range(n_Ts):
    new_filename = "vmd/sistema_{0}.lammpstrj".format(Ts[k])
    data = np.loadtxt(caso+"distribucion_%f.txt" %(Ts[k]), dtype=np.float32)
    L = (1000/0.06)**(1.0/3.0)
